DATE = '2020-04-09' # Date of run
NYT_FILE <- 'data/2020-04-08-covid19-nyt.csv'
...
stan_mod <- stan_model(file='analysis/nb_grw.stan', model_name='nb_grw')
...
state_list = read_csv(NYT_FILE) %>%
 ...
 # dplyr::filter(state %in% c("20", "38", "46", "28", "12", "39", "06", "56", "33", "02", "13", "15", "22", "25", "35", "44")) %>%
...
r <- foreach(i = 1:length(stan_fit_list),
  ...
  new_line <- paste0("'", stan_fit_list[[i]]$sample_file, "',", dur)
  cat(new_line, file = paste0(SAMPLES_ROOT, "/sample-times.csv"), append = TRUE, sep = "\n")
  
             }

total_end_time <- Sys.time()
total_dur <- as.numeric(difftime(total_end_time, total_start_time, units = "secs"))

cat(total_dur, file = paste0(SAMPLES_ROOT,"/", DATE, "-total-time.csv"), append = TRUE, sep = "\n")
stopCluster(cl)

zip(paste0("tmp/", DATE, ".zip"), files = paste0("tmp/", DATE))
